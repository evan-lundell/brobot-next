...
public class GetStatsTests : StatsServiceTestBase
{
    ...
    public async Task ValidDates_ReturnsCorrectCounts()
    {
        ...
        DailyMessageCountRepositoryMock.Setup(dmcr => dmcr.GetTotalDailyMessageCountsByChannel(startDate, endDate, channelId))
            .ReturnsAsync([
        ...
        DailyMessageCountRepositoryMock.Setup(dmcr =>
                dmcr.GetTotalDailyMessageCountsByChannel(startDate, endDate, channelId))
            .ReturnsAsync([
                new DailyMessageCountModel
                {
                    Channel = channelModel,
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(stats.MessageCounts.First().MessageCount, Is.EqualTo(messageCount1 + messageCount2));
            Assert.That(stats.MessageCounts.Last().UserId, Is.EqualTo(user2Id));
            Assert.That(stats.MessageCounts.Last().MessageCount, Is.EqualTo(messageCount3));
