...
{
    public Task<StatPeriodModel?> GetStatPeriodWithCounts(int statPeriodId, CancellationToken cancellationToken = default) =>
        Context.StatPeriods
            .AsSplitQuery()
            .Include(sp => sp.WordCounts)
            .Include(sp => sp.UserMessageCounts)
            .SingleOrDefaultAsync(sp => sp.Id == statPeriodId, cancellationToken);
}